...
// The AI service, same as nginx.conf's /api/ai/ location proxies it. Keeping the dev
// server and the production proxy in step is the whole point of this file — see the
// header note about mirroring nginx.conf.
const AI_PORT = Number(process.env.AI_PORT) || 3001;
...
/** Forward a request to a local backend, preserving method, headers and body. */
function proxyTo(req, res, port) {
  ...
  // A 502 with the reason beats a hung request. The usual cause is the backend having
  // exited, which is visible in this terminal because its output is piped through below.
  upstream.on('error', (err) => {
    res.writeHead(502, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: `backend unreachable on port ${port}: ${err.message}` }));
  });
  req.pipe(upstream);
}
...
const server = createServer((req, res) => {
  ...
  // /api/ai/ must be routed before /api/: nginx's prefix matching gives the longer
  // location precedence, and this mirror has to do the same by hand.
  if (pathname.startsWith('/api/ai/')) {
    proxyTo(req, res, AI_PORT);
    return;
  }
...
/**
 * Start the AI service alongside everything else, the same way startApi does. Without
 * it the assist toggle would be hidden in dev (the SPA probes /api/ai/health), which is
 * not how production behaves. A missing api-ai/node_modules is reported as the
 * actionable thing it is rather than as a stack trace.
 */
function startAi() {
  if (!existsSync(join(ROOT, 'api-ai', 'node_modules'))) {
    console.error('\n  api-ai/node_modules is missing — serving the bundle only.');
    console.error('  Run `npm --prefix api-ai ci` to install it, then restart.\n');
    return null;
  }

  const child = spawn(process.execPath, ['server.js'], {
    cwd: join(ROOT, 'api-ai'),
    env: { ...process.env, PORT: String(AI_PORT) },
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  ...
  child.on('exit', (code) => {
    if (code !== 0 && code !== null) console.error(`  [ai] exited with code ${code}`);
  });
  return child;
}
...
// Leaving the backends running after this process dies orphans a writer on the dev
// database and leaves the AI service bound, and the next `npm run dev` then fails to
// bind the ports for reasons that look unrelated.
const shutdown = () => {
  if (api) api.kill('SIGTERM');
  if (ai) ai.kill('SIGTERM');
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`  ai    http://127.0.0.1:${AI_PORT}  (proxied at /api/ai/)`);
