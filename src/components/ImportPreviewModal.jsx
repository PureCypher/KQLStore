...
//
// The modal chrome (panel, header, row list) lives in ImportPreviewShell, shared with
// the schema import modal; this component owns the parts that are about queries — the
// collision re-planning and the two commit paths.
// ---------------------------------------------------------------------------
...
const ImportPreviewModal = () => {
  ...
  // Plain {index, name, status, reason} rows for the shell. Only the collision rows are
  // re-labelled; everything else is App's verdict, so the two never disagree about the
  // same row. The tooltip on the reason column names the fields an update would move.
  const displayItems = preview.items.map((item, i) => {
    ...
    return {
      ...
      title: detail && detail.changedFields.length > 0
        ? `Changes: ${detail.changedFields.join(', ')}`
        : undefined,
    };
  });
  ...
  return (
    <ImportPreviewShell
      ...
      listAriaLabel="Queries in this import"
      onClose={() => setImportPreview(null)}
      summary={(
        <>
          {/* The counts are the decision the user is being asked to make, so they are
              attached to the confirm button as its description rather than left as four
              coloured numbers floating above it. Each already carries its own word, so
              colour is not the only thing distinguishing them (WCAG 1.4.1). */}
